			...
			try {
				...
				if (!userExists) {
					...
					if (!userExists.avatarKey) {
						const uploadedFile = await utapi.uploadFilesFromUrl(user.image as string);
						if (!uploadedFile.data?.url || !uploadedFile.data?.key) return false;
						await updateUser(userExists.id, userExists.name, uploadedFile.data?.url, uploadedFile.data?.key);
					}
				}
