  ...
  return (
    <div className="w-52 h-20 flex flex-row items-center justify-start gap-2">
      {colors.map((color) => (
        <div
          key={color.id}
          className={`w-[29px] h-[29px] flex justify-center items-center border-2 ${
            color.name === selected.name
              ? "border-slate-400"
              : "border-slate-300"
          } rounded-lg hover:border-slate-400`}
        >
          <div
            className={`w-5 h-5 z-0 rounded-[5px] ${
              color.name
            } active:scale-125 transition duration-150 ${
              color.name === selected.name ? "scale-125" : ""
            }`}
            onClick={() => {
              setSelected(color);
              setState(color.name);
